...
const HelpLine = ({ navigation }) => {
  ...
  useEffect(() => {
    (async () => {
      if (Platform.OS !== "web") {
        const { status } =
          await DocumentPicker.requestDocumentPermissionsAsync();
        if (status !== "granted") {
          alert(
            "Sorry, we need document access permissions to make this work!"
          );
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    if (!location)
      setErrors((errors) => ({ ...errors, location: "Location is required" }));
    if (!landmark)
      setErrors((errors) => ({ ...errors, landmark: "Landmark is required" }));
  ...
  const makePhoneCall = () => {
    const phoneNumber = 9345355391; // Replace with the specific phone number you want to call
  ...
  const handleSubmit = async () => {
    if (
      ...
      try {
        ...
        Alert.alert(
          "Error",
          "An error occurred while trying to send your Complaint."
        );
      }
    } else {
    ...
    >
      <View style={styles.topNavbar}>
        ...
        <View style={styles.userContainer}>
          ...
          <Image
            source={{
              uri: Asset.fromModule(require("../../assets/profile.jpeg")).uri,
            }}
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.checkboxContainer}>
          <TouchableOpacity
            onPress={() => setIsSelling(!isSelling)}
        ...
        <View style={styles.checkboxContainer}>
          <TouchableOpacity
            onPress={() => setIsAddicted(!isAddicted)}
            style={styles.checkbox}
          >
